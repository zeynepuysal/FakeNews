...
class ResourceAllocator:
    # init:  başlatıcı method.
    def __init__(self, num_keys=34, time_window=900, window_limit=15):
        self._lock = Lock() # lock: çoklu işlemler arasında güvenli paylaşımı sağlar.
        self.val = None # val: mevcut olan kaynak sayısını temsil eder.
        self.num_keys = num_keys
        self.timers = dict() # timers: her bir anahtar için zaman ölçümlerini tutar.
    ...
    def change_params(self, window_limit, time_window):
        ...
        self.window_limit = window_limit
    """
    Kullanılabilir bir kaynağın indeksini döndürür veya hiçbir kaynak kullanılabilir değilse kaynakların ne kadar süreyle
    kullanılamayacağını belirtir.

    Returns:
    - int: Kullanılabilir bir kaynağın indeksi veya hiçbir kaynak kullanılabilir değilse, kullanılamayacak süre (negatif değer).
    """
